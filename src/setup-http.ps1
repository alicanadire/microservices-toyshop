# EShop Mikroservices - TAM HTTP KURULUM
...
try {
    ...
    # Port temizleme
    $ports = @(5000, 6005, 6004, 6000, 6001, 6002, 6003, 5432, 5433, 5434, 1433, 6379, 5672, 15672)
...
"@
    ...
    # Docker build - HTTP only
    Write-Host "`n🏗️ HTTP-only Docker build başlıyor..." -ForegroundColor Cyan
    ...
    Write-Host "2️⃣ Identity Service build (HTTP only)..." -ForegroundColor Yellow
    ...
    Write-Host "5️⃣ Diğer servisler build..." -ForegroundColor Yellow
    ...
    # Servisleri başlat
    Write-Host "`n🚀 HTTP servisleri başlatılıyor..." -ForegroundColor Cyan
    docker-compose up -d
    
    if ($LASTEXITCODE -ne 0) { throw "Services startup failed" }
    
    # Servislerin hazır olmasını bekle
    Write-Host "`n⏳ HTTP servisleri hazırlanıyor..." -ForegroundColor Cyan
    $maxWait = 90
    $waited = 0
    ...
    while ($waited -lt $maxWait) {
        ...
        if (Test-HttpService "Identity Service" "http://localhost:5000") { $readyCount++ }
        if (Test-HttpService "Shopping Web" "http://localhost:6005") { $readyCount++ }
        if (Test-HttpService "API Gateway" "http://localhost:6004") { $readyCount++ }
        
        if ($readyCount -eq 3) {
            Write-Host "🎉 Ana HTTP servisleri hazır!" -ForegroundColor Green
            break
        }
    ...
    # Final kontrol
    Write-Host "`n📊 HTTP sistem durumu:" -ForegroundColor Cyan
    docker-compose ps
    
    # Başarı mesajı
    Write-Host @"

🎉 HTTP-ONLY KURULUM BAŞARIYLA TAMAMLANDI!

🌐 HTTP ERİŞİM LİNKLERİ (SSL YOK!):
...
✅ TÜM SİSTEM HTTP-ONLY ÇALIŞIR DURUMDA!
❌ HTTPS YOK, SSL YOK, KARIŞIKLIK YOK!
...
"@ -ForegroundColor Green
    ...
    Write-Host "4. HTTP portlar kullanımda mı?" -ForegroundColor White
...
Write-Host "`nHTTP-Only script tamamlandı!" -ForegroundColor Gray
Read-Host "Çıkmak için Enter'a basın..."
